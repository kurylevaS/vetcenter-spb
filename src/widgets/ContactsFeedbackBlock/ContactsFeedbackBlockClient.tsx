...
interface IContactsFeedbackBlockClientProps {
  title: string;
  description: string;
  buttonText: string;
}

const ContactsFeedbackBlockClient = ({
  title,
  description,
  buttonText,
}: IContactsFeedbackBlockClientProps) => {
  return (
      ...
      <div className="xl:max-w-[1440px] w-full mx-auto px-4 md:px-8 lg:px-16">
        <div className="bg-cGreen rounded-[2.4rem] md:rounded-[3.2rem] lg:rounded-[3.2rem] xl:rounded-[4rem] p-8 md:p-12 lg:p-16 xl:p-20 relative overflow-hidden">
          ...
          <div className="relative z-10 w-full">
            <FeedbackForm buttonText={buttonText} />
          </div>
        </div>
      </div>
    </section>
...
export default ContactsFeedbackBlockClient;
